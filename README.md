...
This means, from now on, TensorFlow code can be used and debugged as ordinary python code (using numpy for example). This is just one aspect of making TensorFlow more pythonic.

Below there are two tasks. I would highly recommend doing them because while watching me code as well as coding yourself, you'll definitely internalize the material.
...
Now you might think, so what? Just some imports changed, Big deal. But this is only one part of the story. Yes, the imports changed, and as you can see later in the example, you can basically leave your existing Keras code intact most of the times and just change the import and you're done.

Also, in addition, Keras can now make use of built-in TensorFlow functionality which wasn't possible before. For example, you can take your 1:1 Keras code and TensorFlow will scale it to a large GPU or a TPU cluster. We'll have a look at this in the next chapter.

For now, just follow along the video and attempt the coding exercise below to get an idea how things work:
