...
Lack of eager execution was one of the main complaints against TensorFlow. We can all relate. Having to execute the whole graph and then trying to debug based on the errors was very tedious. Especially, since values of intermediate results weren't accessible without printing them out, mixing in debug statements into the production code.
